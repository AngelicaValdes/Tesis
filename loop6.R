...
for (j in clientes$child_id[(count_clientes+1):(677)]){
  ...
  unlink('H:/Dropbox/yan shang/delays angelica/mydata_server_13.RData', recursive = TRUE)
  save.image(file = 'H:/Dropbox/yan shang/delays angelica/mydata_server_13.RData')
  rm(list = ls())
  gc()
  load('H:/Dropbox/yan shang/delays angelica/mydata_server_13.RData')
