...
DB1<-read_excel("jerarquico_theta_desv_theta_ruta1.xlsx" , guess_max = 50000)
DB2<-read_excel("jerarquico_tau_desv_theta_ruta1.xlsx" , guess_max = 50000)
#DB1<-read_excel("jerarquico_theta_desv_tau.xlsx" , guess_max = 50000)
#DB2<-read_excel("jerarquico_tau_desv_tau.xlsx" , guess_max = 50000)
...
for (j in clientes$client[1:677]){
  ...
  x_mod_1<-c(cliente2$weight_kg_1[nperiod],theta$sigma_mod[1],theta$xi_mod[1],tau$tau_mod[1],
             tau$sigma_mod[1],tau$xi_mod[1],cliente2$demand_1[nperiod],
             cliente2$first_half_week_1[nperiod],theta$theta_mod[1]/cliente2$distance_1[nperiod])
  ...
  if (nRoutes >= 2){
    ...
    x_mod_2<-c(cliente2$weight_kg_2[nperiod],theta$sigma_mod[2],theta$xi_mod[2],tau$tau_mod[2],
               tau$sigma_mod[2],tau$xi_mod[2],cliente2$demand_2[nperiod],
               cliente2$first_half_week_2[nperiod],theta$theta_mod[2]/cliente2$distance_2[nperiod])
  ...
  if (nRoutes >= 3){
    ...
    x_mod_3<-c(cliente2$weight_kg_3[nperiod],theta$sigma_mod[3],theta$xi_mod[3],tau$tau_mod[3],
               tau$sigma_mod[3],tau$xi_mod[3],cliente2$demand_3[nperiod],
               cliente2$first_half_week_3[nperiod],theta$theta_mod[3]/cliente2$distance_3[nperiod])
  ...
  if (nRoutes >= 4){
    ...
    x_mod_4<-c(cliente2$weight_kg_4[nperiod],theta$sigma_mod[4],theta$xi_mod[4],tau$tau_mod[4],
               tau$sigma_mod[4],tau$xi_mod[4],cliente2$demand_4[nperiod],
               cliente2$first_half_week_4[nperiod],theta$theta_mod[4]/cliente2$distance_4[nperiod])
  ...
  x<-x+9 #11 es el numero de variables
  pos_r<-pos_r+nRoutes
}

save.image("jerarquico_theta_tau_desv_theta_ruta1.RData")
library(writexl)
write_xlsx(efecto, "H:/Dropbox/yan shang/delays angelica/jerarquico_theta_tau_desv_theta_ruta1.xlsx")
