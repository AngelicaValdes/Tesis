...
for (j in clientes$child_id[(count_clientes+1):(677)]){
  ...
  unlink('H:/Dropbox/yan shang/delays angelica/mydata_server_3.RData', recursive = TRUE)
  save.image(file = 'H:/Dropbox/yan shang/delays angelica/mydata_server_3.RData')
  rm(list = ls())
  gc()
  load('H:/Dropbox/yan shang/delays angelica/mydata_server_3.RData')
  files <- list.files("C:\\Users\\angelica\\AppData\\Local\\Temp\\3", full.names = T, pattern = "Rtmp")
