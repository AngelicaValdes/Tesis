...
for (j in clientes$child_id[(count_clientes+1):(677)]){
  ...
  for (i in 1:h){
    ...
    if (nRoutes >= 1){taur$tau1[i]<-summary[4]}
    if (nRoutes >= 2){taur$tau2[i]<-summary[5]}
    if (nRoutes >= 3){taur$tau3[i]<-summary[6]}
    if (nRoutes >= 4){taur$tau4[i]<-summary[7]}
  ...
  Base_pronostico$tau[1+f]<-param[1]
  ...
  for(i in 1:h){
    ...
    Base_pronostico$tau[d+1+f]<-tau[i]
  ...
  for(i in 2:(max(BD$Periodo_final)+1)){
    if(is.na(Base_pronostico$tau[i+f]))Base_pronostico$tau[i+f]<-Base_pronostico$tau[i-1+f]
